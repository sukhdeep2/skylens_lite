...
class hankel(object):
    def __init__(
        self, x, fx, nu, N_extrap_low=0, N_extrap_high=0, c_window_width=0.25, N_pad=0
    ):
        print("nu is required to be between (0.5-n) and 2.")
        self.myfftlog = fftlog(
            x, np.sqrt(x) * fx, nu, N_extrap_low, N_extrap_high, c_window_width, N_pad
        )
...
def g_m_vals(mu, q):
    ...
    g_m[(np.absolute(imag_q) + np.absolute(mu) <= cut) & (q != mu + 1 + 0.0j)] = gamma(
        alpha_plus
    ) / gamma(alpha_minus)
    ...
    g_m[np.absolute(imag_q) + np.absolute(mu) > cut] = np.exp(
        ...
        - asym_q
        + 1.0 / 12 * (1.0 / asym_plus - 1.0 / asym_minus)
        + 1.0 / 360.0 * (1.0 / asym_minus**3 - 1.0 / asym_plus**3)
        + 1.0 / 1260 * (1.0 / asym_plus**5 - 1.0 / asym_minus**5)
    )
...
def g_m_ratio(a):
    ...
    # asymptotic form
    g_m[np.absolute(imag_a) > cut] = np.exp(
        (asym_a - 0.5) * np.log(asym_a)
        - asym_a * np.log(asym_a_plus)
        + 0.5
        + 1.0 / 12 * (1.0 / asym_a - 1.0 / asym_a_plus)
        + 1.0 / 360.0 * (1.0 / asym_a_plus**3 - 1.0 / asym_a**3)
        + 1.0 / 1260 * (1.0 / asym_a**5 - 1.0 / asym_a_plus**5)
    )
...
def g_l_1(l, z_array):
    ...
    q = z_array - 2.5
    """
    gl1 = -(2.0 ** (z_array - 1)) * (z_array - 1) * g_m_vals(l + 0.5, z_array - 2.5)
    return gl1
...
def h_l(l, z_array):
    ...
    """
    hl = g_m_vals(2 * l + 1.0, z_array - 2.0) * g_m_ratio((2.0 - z_array) / 2.0)
    return hl
