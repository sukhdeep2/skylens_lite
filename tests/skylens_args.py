...
cl_func_names = (
    {}
)  # if we want to use custom functions for cl computations. must be in globals()
...
zkernel_func_names = (
    {}
)  # if we want to use custom functions for redshift kernels. must be in globals()
...
clean_tracer_window = (
    True  # remove tracer windows from memory once coupling matrices are done
)
...
nzbins = 2
shear_zbins = lsst_source_tomo_bins(
    nbins=nzbins,
...
cosmo_params["Oml"] = 1.0 - cosmo_params["Om"] - cosmo_params["Omk"]
