...
Nl_bins=20
l_bins=jnp.int32(jnp.logspace(jnp.log10(lmin_cl_Bins),jnp.log10(lmax_cl_Bins),Nl_bins))
lb=jnp.sqrt(l_bins[1:]*l_bins[:-1])

l=jnp.unique(jnp.int32(jnp.logspace(jnp.log10(lmin_cl),jnp.log10(lmax_cl),Nl_bins*20))) #if we want to use fewer ell

bin_cl=True

bin_xi=True
theta_bins=jnp.logspace(jnp.log10(1./60),1,20)
        ...
        for use_window in use_windows:
                    for use_binned_l in use_binned_ls:
                        for store_win in store_wins:
                            ...
                            print("\n","\n")
                            try:
                                kappa0=Skylens(shear_zbins=zs_bin1,bin_cl=bin_cl,l_bins=l_bins,l=l0, galaxy_zbins=zs_bin1,
                                               use_window=use_window,
                                               use_binned_l=use_binned_l,wigner_files=wigner_files,
                                               store_win=store_win,window_lmax=window_lmax,
                                               corrs=corrs,do_xi=do_xi,bin_xi=bin_xi,theta_bins=th_bins,WT=WT,
                                                use_binned_theta=use_binned_l,scheduler_info=scheduler_info
                                               )
                                if do_xi:
                                    G=kappa0.xi_tomo()
...
#                                 kappa0.WT.gather_data()
                            ...
                            LC,scheduler_info=start_client(Scheduler_file=None,local_directory='../temp/'+str(nid)+'/',ncpu=None,n_workers=1,     
                                                                threads_per_worker=None,memory_limit='120gb',dashboard_address=8801)
                            nid+=1
                            client=client_get(scheduler_info=scheduler_info)
                            client.wait_for_workers(n_workers=1)
