...
wlmax = 4400  # This needs to be atleast 2X lmax in general, unless you are certain window is narrow in ell space.
...
def wig3j_recur_2d(j1b, m1, m2, m3, j3_outmax, step, j2b):
    ...
    if j2b < j1b:  # we exploit j1-j2 symmetry and hence only compute for j2>=j1
        return [j1b, j2b, 0]
    if (
        np.absolute(j2b - j1b - step - 1) > j3_outmax
...
for lb1 in lb:
    ...
    for lb2 in lb:
        ...
        out = out[4]
        z1[
            :, lb1 : lb1 + step, lb2 : lb2 + step
        ] = out  # cannot write in parallel, race conditions
        # for j1i in np.arange(len(j1)):
        #   for j2i in np.arange(len(j2)):
        #      if j2[j2i]==j1[j1i]:
        #         out[:,j1i,j2i]*=0 #don't want to add diagonal twice below.
        z1[:, lb2 : lb2 + step, lb1 : lb1 + step] = out.transpose(
            0, 2, 1
