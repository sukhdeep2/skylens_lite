...
class PCA_shear:
    ...
    def compute_modelv(self, cosmo_params=cosmo_fid, do_cov=False, scenario=None):
        ...
        clSG = self.kappa_class.cl_tomo(
            pk_params=pk_params2, corrs=[("shear", "shear")]
        )
    ...
    def compute_mock_obs_data(self, scenario=None, cosmo_params=None):
        ...
        clS_dmo = self.compute_modelv(
            cosmo_params=cosmo_params, do_cov=False, scenario="dmo"
        )
    ...
    def cal_invL(self, COV):
        ...
        if np.linalg.det(COV) == 0:
            ...
            cov_F = 1.0 / np.exp(logD / len(COV))
            print(
                "COV rescaled for Cholesky... check the results. ",
                cov_F,
                np.linalg.det(COV),
                np.linalg.det(COV * cov_F),
            )
    ...
    def build_Ratio(self):
        ...
        for j in range(self.Nscenario):
            self.modelv_bary_fid[self.PCA_simName[j]] = self.compute_modelv(
                scenario=self.PCA_simName[j], do_cov=False
            )["cl_b"]
            self.Ratio.T[j] = (
                self.modelv_bary_fid[self.PCA_simName[j]] / self.modelv_dmo_fid
            )

        self.Ratio_1 = self.Ratio - 1.0
    ...
    def cal_Qexp(self, scenario):
        modelv_bary_fid = self.compute_modelv(
            cosmo_params=cosmo_fid, do_cov=False, scenario=scenario
        )["cl_b"]
    ...
    def compute_modelv_bary(self, cosmo_params=cosmo_fid, Q=None):
        ...
        modelv_dmo = self.compute_modelv(
            cosmo_params=cosmo_params, do_cov=False, scenario="dmo"
        )["cl_b"]
