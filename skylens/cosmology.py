...
class cosmology:
    ...
    def set_cosmology(self, cosmo_params=None):
        ...
        if self.Omk != 0:
            self.Dh = self.c * (
                jnp.sqrt(jnp.absolute(self.Omk))
            )  # a0h0=1./sqrt(Omega_k)
        if self.h_inv:
            self.Dh = self.c / 100.0
    ...
    def set_astropy(self, cosmo_params=None, cosmo_h=None):
        ...
        m_nu = jnp.array(self.astropy_cosmo.m_nu.value)
        m_nu = m_nu.at[-1].set(cosmo_params["mnu"])
        m_nu *= self.astropy_cosmo.m_nu.unit
        self.astropy_cosmo = self.astropy_cosmo.clone(
        ...
        if cosmo_params.get("w0") is not None:
            ...
            if cosmo_params["w0"] != -1 or cosmo_params["wa"] != 0:
                self.astropy_cosmo = self.astropy_cosmo_w0_wa(
                    cosmo=self.astropy_cosmo, w0=cosmo_param["w0"], wa=cosmo_param["wa"]
                )
        if self.h_inv:
    ...
    def E_z(self, z):
        ...
        return jnp.sqrt(
            ...
            + self.Omk * (z1) ** 2
            + self.OmR * (z1) ** 4
            + self.OmL * (z1) ** (3 * (1 + self.w))
        )

    def E_z_inv(self, z, Ez_func=None):  # 1/E_z
        if not Ez_func:
            Ez_func = self.E_z
        return 1.0 / Ez_func(z)
        ...
        if hasattr(z, "__len__"):
            ...
            for i in z:
                try:
                    ...
                    DE_int = scipy_int1d(
                        DE_int_funct, 0, i, epsrel=self.rtol, epsabs=tol
                    )[0]
                ez[j] = jnp.sqrt(
                    self.Om * (1 + i) ** 3
                    + self.Omk * (1 + i) ** 2
                    + self.OmR * (1 + i) ** 4
                    + self.OmL * jnp.exp(DE_int)
                )
            ...
            try:
                ...
                DE_int = scipy_int1d(DE_int_funct, 0, z, epsrel=self.rtol, epsabs=tol)[
                    0
                ]
        ...
        if hasattr(z, "__len__"):
            ...
            for i in z:
                try:
                    ...
                    ezi = scipy_int1d(self.E_z_inv, 0, i, epsrel=self.rtol, epsabs=tol)[
                        0
                    ]  # integration for array (vector) of z
                if len(z) == 1:
                    ez = ezi
                else:
                    ez[j] = ezi
            ...
            try:
                ...
                ez = scipy_int1d(self.E_z_inv, 0, z, epsrel=self.rtol, epsabs=tol)[
                    0
                ]  # integration for scalar z
        return ez * self.Dh

    def comoving_distance_trapz(
        self, z=[0]
    ):  # line of sight comoving distance... computed as sum
    ...
    def comoving_transverse_distance_z1z2(
        self, z1=[0], z2=[]
    ):  # transverse comoving distance between 2 redshift
    ...
    def DZ_int(
        ...
        for i in z:
            try:
                ...
                dz[j] = (
                    self.H_z(i)
                    * scipy_int1d(intf, i, inf, epsrel=self.rtol, epsabs=tol)[0]
                )
            j = j + 1
    ...
    def Omega_Z(self, z=[0]):
        z = jnp.array(z)
        omz = (
            (self.H0**2 / self.H_z(z) ** 2) * self.Om * (1 + z) ** 3
        )  # omega_z=rho(z)/rho_crit(z)
        return omz
    ...
    def f_z0(
        self, z=[0]
    ):  # from full func f(z)=a/D(dD/da), =-(1+z)dD/dz.. full integral here but slow.. better to interpolate self.fz
        fz = jnp.zeros_like(z)
        j = 0
        for i in z:
            z2 = jnp.linspace(i - 0.01, i + 0.01, 101)
            Dz = self.DZ_int(z=z2)
            f2 = -1.0 * (1 + z2) / (Dz) * jnp.gradient(Dz, z2[1] - z2[0])
            fz[j] = f2[50]
            j += 1
        return fz
...
class EH_pk:  # eisenstein_hu power spectra https://arxiv.org/pdf/astro-ph/9709112.pdf
    ...
    def z_eq(self):
        self.z_eq = 2.5 * 1.0e4 * self.Om_h2 * (self.theta**-4)
    ...
    def z_drag(self):
        ...
        b2 = 0.238 * (self.Om_h2) ** 0.223
        self.zd = (
            1291
            / (1 + 0.659 * (self.Om_h2) ** 0.828)
            * (self.Om_h2**0.251)
            * (1 + b1 * self.Ob_h2**b2)
        )
    ...
    def s_drag(self):  # sound horizon
        self.sd = (
            ...
            / 3
            / self.k_eq
            * jnp.sqrt(6 / self.R_eq)
            * jnp.log(
                (jnp.sqrt(1 + self.Rd) + jnp.sqrt(self.Rd + self.R_eq))
                / (1 + jnp.sqrt(self.R_eq))
            )
        )
    ...
    def transfer_cdm(self):
        ...
        b1 = 0.944 / (1 + (458 * self.Om_h2) ** -0.708)
        #         b2 = (0.395*self.Om_h2)**-.0266 #this is in paper.
        b2 = 0.395 * self.Om_h2**-0.0266  # from nbodykit
        beta_c = 1 / (1 + b1 * ((1 - bf) ** b2) - 1)
        f = 1.0 / (1 + (self.k * self.sd / 5.4) ** 4)
        self.cdm_f = f
        self.transfer_c = f * self.transfer_0(1, beta_c) + (1 - f) * self.transfer_0(
            alpha_c, beta_c
        )
    ...
    def transfer_baryon(self):
        ...
        beta_b = (
            0.5
            + self.Omb / self.Om
            + (3 - 2 * self.Omb / self.Om) * jnp.sqrt(1 + (17.2 * self.Om_h2) ** 2)
        )
        self.beta_b = beta_b

        ks = self.k * self.sd
        s_app = self.sd / (1 + (beta_n / ks) ** 3) ** (1.0 / 3)

        self.transfer_b = self.transfer_0(1, 1) / (1 + (ks / 5.2) ** 2) + alpha_b / (
            1 + (beta_b / ks) ** 3
        ) * jnp.exp(-((self.k / self.ksilk) ** 1.4))
...
sigma_crit_norm100 = (3.0 / 2.0) * ((H100 / c) ** 2) / Rho_crit100
