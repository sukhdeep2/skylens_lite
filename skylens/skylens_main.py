...
class Skylens():
    def __init__(self,use_defaults=True,yaml_inp_file=None,python_inp_file=None,
                ...
                Ang_PS=None,logger=None,tracer_utils=None,
        ...
        if Ang_PS is None:
            self.Ang_PS=Angular_power_spectra(
                                l=self.l_cl,logger=self.logger,
...
#                 else:
    ...
    def set_binned_measure(self,local_args,clean_up=False): #FIXME: needs cleanup, refactoring
        """
...
#                         pcl_b[corr][(i,j)]=delayed(calc_pseudo_cl)(None,cl_b[corr][(i,j)],Win=self.Win.Win['cl'][corr][(i,j)])
        ...
        out_stack=delayed(self.stack_dat)({'pcl_b':pcl_b,'est':'pcl_b'},corrs=corrs,
                                          corr_indxs=stack_corr_indxs,stack_file_write=stack_file_write)
        return {'stack':out_stack,'cl_b':cl_b,'cl':cl,'pseudo_cl':pcl,
                ...
                if self.use_window and self.do_pseudo_cl:
                    if not self.bin_window:
                        cl=self.cl_func[corr](zbin1=zkernel[corr[0]][i],zbin2=zkernel[corr[1]][j],
                                                             corr=corr,cosmo_params=cosmo_params,Ang_PS=Ang_PS) #clz=Ang_PS.clz)
...
#                                                         s1_s2=s1_s2,Win=win)
        ...
        out['stack']=delayed(self.stack_dat)({'xi':xi,'est':'xi'},corrs=corrs,
