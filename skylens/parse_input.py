...
def set_l_bins(skylens_kwargs={}):
    ...
    if skylens_kwargs["l"] is None:
        skylens_kwargs["l"] = np.arange(
            skylens_kwargs["l_bin_min"], skylens_kwargs["l_bin_max"]
        )
    if skylens_kwargs["l_bins"] is None:
        if log_bins:
            skylens_kwargs["l_bins"] = np.logspace(
                np.log10(skylens_kwargs["l_bin_min"]),
                np.log10(skylens_kwargs["l_bin_max"]),
                skylens_kwargs["Nl_bins"] + 1,
            )
        else:
            skylens_kwargs["l_bins"] = np.linspace(
                skylens_kwargs["l_bin_min"],
                skylens_kwargs["l_bin_max"],
                skylens_kwargs["Nl_bins"] + 1,
            )
...
def set_theta_bins(skylens_kwargs={}):
    ...
    if skylens_kwargs["theta_bins"] is None:
        if log_bins:
            skylens_kwargs["theta_bins"] = np.logspace(
                np.log10(skylens_kwargs["theta_min"]),
                np.log10(skylens_kwargs["theta_max"]),
                skylens_kwargs["n_theta_bins"] + 1,
            )
        else:
            skylens_kwargs["theta_bins"] = np.linspace(
                skylens_kwargs["theta_min"],
                skylens_kwargs["theta_max"],
                skylens_kwargs["n_theta_bins"] + 1,
            )
    return skylens_kwargs
...
def set_WT(skylens_kwargs={}):
    ...
    if skylens_kwargs["WT_kwargs"].get("theta") is None:
        if log_bins:
            skylens_kwargs["WT_kwargs"]["theta"] = np.logspace(
                np.log10(skylens_kwargs["theta_min"] * 0.99),
                np.log10(skylens_kwargs["theta_max"] * 1.01),
                skylens_kwargs["n_theta_bins"] * 10,
            )
        else:
            skylens_kwargs["WT_kwargs"]["theta"] = np.linspace(
                skylens_kwargs["theta_min"] * 0.99,
                skylens_kwargs["theta_max"] * 1.01,
                skylens_kwargs["n_theta_bins"] * 10,
            )
    s1_s2_new = []
...
def set_cosmology(skylens_kwargs={}):
    ...
    if skylens_kwargs["cosmo_params"]["read_cosmo"] is not None:
        ...
        skylens_kwargs["cosmo_params"].update(
            {
                "h": cosmo.h,
                "Omb": cosmo.Ob0,
...
def set_cluster(skylens_kwargs={}):
    ...
    skylens_kwargs["scheduler_args"]["local_directory"] += (
        "/pid" + str(os.getpid()) + "/"
    )
