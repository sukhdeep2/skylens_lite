...
nz_F = 3600.0 / d2r**2  # convert nz from arcmin^2 to rd ^2


def galaxy_shot_noise_calc(zg1=None, zg2=None):
    SN = jnp.sum(zg1["W"] * zg2["W"] * zg1["nz"] * nz_F)  # FIXME: Check this
    # Assumption: ns(z)=ns*pzg*dzg
    SN /= jnp.sum(zg1["nz"] * nz_F * zg1["W"])
    SN /= jnp.sum(zg2["nz"] * nz_F * zg2["W"])
    return SN
...
def shear_shape_noise_calc(zs1=None, zs2=None, sigma_gamma=0):
    ...
    SN = SN0 * jnp.sum(
        zs1["W"] * zs2["W"] * zs1["nz"] * nz_F
    )  # FIXME: this is probably wrong.
    # Assumption: ns(z)=ns*pzs*dzs
    SN /= jnp.sum(zs1["nz"] * nz_F * zs1["W"])
    SN /= jnp.sum(zs2["nz"] * nz_F * zs2["W"])
    #     print(zs1['ns'])
    return SN


def lsst_pz_source(
    alpha=2, z0=0.11, beta=0.68, z=[]
):  # alpha=1.24,z0=0.51,beta=1.01,z=[]
    p_zs = z**alpha * jnp.exp(-((z / z0) ** beta))
    p_zs /= jnp.sum(jnp.gradient(z) * p_zs) if len(z) > 1 else 1
    return p_zs
...
def ztrue_given_pz_Gaussian(zp=[], p_zp=[], bias=[], sigma=[], zs=None):
    ...
    """
    if zs is None:
        zs = jnp.linspace(min(zp - sigma * 5), max(zp - sigma * 5), 500)

    y = jnp.tile(zs, (len(zp), 1))

    pdf = gaussian.pdf(y.T, loc=zp + bias, scale=sigma).T

    dzp = jnp.gradient(zp) if len(zp) > 1 else 1

    p_zs = jnp.dot(dzp * p_zp, pdf)

    dzs = jnp.gradient(zs)
    # print(p_zs,dzp,p_zp,pdf,zp,bias)
    if jnp.sum(p_zs * dzs) > 0:
        p_zs /= jnp.sum(p_zs * dzs)
    else:
        p_zs[:] = 0
...
def set_window(
    ...
    npix = jnp.int32(npix0 * f_sky)
    if mask is None:
        mask = jnp.zeros(npix0, dtype="bool")
        #     mask[int(npix):]=0
        mask = mask.at[mask_start_pix : mask_start_pix + int(npix)].set(1)

    cl_map0 = hp.ma(jnp.ones(npix0))
    ...
    for i in range(shear_zbins["n_bins"]):
        cl_i = client.compute(cl0G["cl"][corr][(i, i)]).result()
        if jnp.any(jnp.isnan(cl_i)):
            print("survey utils, set_window:", cl_i)
            crash
        if unit_win:
            cl_map = hp.ma(jnp.ones(12 * nside * nside))
        #             cl_i=1
        else:
            cl_i += shear_zbins["SN"]["galaxy"][-1, i, i]
            if window_cl_fact is not None:
                cl_i *= window_cl_fact
            cl_map = hp.ma(1 + hp.synfast(np.array(cl_i), nside=nside))
        cl_map[cl_map <= 0] = 1.0e-4
        cl_map[~mask] = hp.UNSEEN
        cl_t = hp.anafast(cl_map)
        #         cl_map/=cl_map[mask].mean()
        #         if not unit_win:
        #             cl_map/=jnp.sqrt(cl_t[0]) #this is important for shear map normalization in correlation functions.
        cl_map[~mask] = hp.UNSEEN
        cl_map_noise = jnp.sqrt(cl_map)
        cl_map_noise = cl_map_noise.at[~mask].set(hp.UNSEEN)
        # cl_map.mask=mask
        shear_zbins[i]["window_cl0"] = cl_i
        shear_zbins[i]["window"] = cl_map
        if delta_W:
            shear_zbins[i]["window_N"] = jnp.sqrt(shear_zbins[i]["window"])
            shear_zbins[i]["window_N"] = (
                shear_zbins[i]["window_N"].at[~mask].set(hp.UNSEEN)
            )
        else:
            print("not using delta_W window")
            shear_zbins[i]["window_N"] = jnp.sqrt(1.0 / shear_zbins[i]["window"])
            shear_zbins[i]["window_N"] = (
                shear_zbins[i]["window_N"].at[~mask].set(hp.UNSEEN)
            )
            shear_zbins[i]["window"] = shear_zbins[i]["window"].at[:].set(1)
            shear_zbins[i]["window"] = shear_zbins[i]["window"].at[~mask].set(hp.UNSEEN)
...
def zbin_pz_norm(
    ...
    dzs = jnp.gradient(zs) if len(zs) > 1 else 1

    x = jnp.absolute(p_zs) < 1.0e-10  # to avoid some numerical errors
    p_zs = p_zs.at[x].set(0)

    if jnp.sum(p_zs * dzs) != 0:
        p_zs = p_zs / jnp.sum(p_zs * dzs)
    else:
    ...
    shear_zbins[i]["dz"] = (
        jnp.gradient(shear_zbins[i]["z"]) if len(shear_zbins[i]["z"]) > 1 else 1
    )
    ...
    shear_zbins[i]["Norm"] = jnp.sum(shear_zbins[i]["pzdz"])
    shear_zbins[i]["b1"] = bg1
    shear_zbins[i]["bz1"] = bg1 * jnp.ones_like(shear_zbins[i]["pz"])
    shear_zbins[i]["AI"] = AI
    shear_zbins[i]["AI_z"] = AI_z
    shear_zbins[i]["mag_fact"] = mag_fact
    shear_zbins[i][
        "shear_m_bias"
    ] = shear_m_bias  # default value is 1. This is really 1+m
    zm = jnp.sum(shear_zbins[i]["z"] * shear_zbins[i]["pzdz"]) / shear_zbins[i]["Norm"]
    shear_zbins[i]["lm"] = k_max * cosmo_h.comoving_transverse_distance(zm).value
    shear_zbins[i]["k_max"] = k_max
    return shear_zbins


def source_tomo_bins(
    zp=None,
    p_zp=None,
    nz_bins=None,
    ns=26,
    ...
    **kwargs
    ...
    if z_bins is None:
        z_bins = jnp.linspace(min(zp) - 0.0001, max(zp) + 0.0001, nz_bins + 1)

    if zs is None:
        sigma_max = 0
        if zp_sigma is not None:
            sigma_max = max(jnp.atleast_1d(zp_sigma)) * 5
        #         zs=jnp.linspace( max(0.01,min(zp)-sigma_max), max(zp)+sigma_max,len(zp)+2)#n_zs)
        print(zp, n_zs, z_true_max)
        zs = jnp.linspace(min(zp), z_true_max, n_zs)
        zs = zs[zs <= z_true_max]
    ...
    zl_kernel = jnp.linspace(0, max(zs), 50)
    lu = Tracer_utils()
    cosmo_h = cosmo_h_PL
    ...
    shear_zbins["SN"] = {}
    shear_zbins["SN"]["galaxy"] = jnp.zeros((len(l), nz_bins, nz_bins))
    shear_zbins["SN"]["shear"] = jnp.zeros((len(l), nz_bins, nz_bins))
    shear_zbins["SN"]["kappa"] = jnp.zeros((len(l), nz_bins, nz_bins))
    ...
    for i in range(nz_bins):
        ...
        if ztrue_func is None:
            ...
            ns_i = ns * jnp.sum(p_zp[indx[0] : indx[1]] * dzp[indx[0] : indx[1]])
            zs, p_zs = ztrue_func(
                zp=zp[indx[0] : indx[1]],
                p_zp=p_zp[indx[0] : indx[1]],
                bias=zp_bias[indx[0] : indx[1]],
                sigma=zp_sigma[indx[0] : indx[1]],
                zs=zs,
            )

        shear_zbins = zbin_pz_norm(
            shear_zbins=shear_zbins,
            bin_indx=i,
            zs=zs,
            p_zs=p_zs,
            ns=ns_i,
        ...
        # shear_zbins[i]['lens_kernel']=jnp.dot(shear_zbins[i]['pzdz'],sc)
    ...
    #     for i in range(ib,nz_bins):
    #         del shear_zbins[i]
    #     nz_bins-=len(pop_keys)

    for i in range(nz_bins):
        if use_shot_noise:
            shear_zbins["SN"]["galaxy"] = (
                shear_zbins["SN"]["galaxy"]
                .at[:, i, i]
                .set(galaxy_shot_noise_calc(zg1=shear_zbins[i], zg2=shear_zbins[i]))
            )
            shear_zbins["SN"]["shear"] = (
                shear_zbins["SN"]["shear"]
                .at[:, i, i]
                .set(
                    shear_shape_noise_calc(
                        zs1=shear_zbins[i], zs2=shear_zbins[i], sigma_gamma=sigma_gamma
                    )
                )
            )
            shear_zbins["SN"]["kappa"] = (
                shear_zbins["SN"]["kappa"]
                .at[:, i, i]
                .set(
                    shear_shape_noise_calc(
    ...
    if use_window:
        shear_zbins = set_window(
            shear_zbins=shear_zbins,
            f_sky=f_sky,
            nside=nside,
            mask_start_pix=mask_start_pix,
...
def lens_wt_tomo_bins(
    ...
    z_bins = jnp.linspace(min(zp), max(zp) * 0.9, nz_bins) if z_bins is None else z_bins

    shear_zbins0 = source_tomo_bins(
        zp=zp,
        p_zp=p_zp,
        zp_bias=zp_bias,
        zp_sigma=zp_sigma,
    ...
    zl = jnp.linspace(0, 2, 50)
    sc = 1.0 / lu.sigma_crit(zl=zl, zs=zs, cosmo_h=cosmo_h)
    scW = 1.0 / jnp.sum(sc, axis=1)

    for i in range(nz_bins):
        i = jnp.int(i)
        shear_zbins[i] = copy.deepcopy(shear_zbins0[0])
        shear_zbins[i]["W"] = 1.0 / lu.sigma_crit(zl=z_bins[i], zs=zs, cosmo_h=cosmo_h)
        shear_zbins[i]["W"] *= scW
        shear_zbins[i]["pz"] *= shear_zbins[i]["W"]

        x = (
            shear_zbins[i]["pz"] > -1
        )  # 1.e-10 # FIXME: for shape noise we check equality of 2 z arrays. Thats leads to null shape noise when cross the bins in covariance
        ...
        shear_zbins[i]["lens_kernel"] = (
            jnp.dot(shear_zbins[i]["pzdz"], sc) / shear_zbins[i]["Norm"]
        )
    shear_zbins["n_bins"] = nz_bins  # easy to remember the counts
...
def galaxy_tomo_bins(
    ...
    if zg is None:
        zg = jnp.linspace(0, 1.5, 100)
    dzg = jnp.gradient(zg)
    dzp = jnp.gradient(zp) if len(zp) > 1 else [1]
    zp = jnp.array(zp)

    zl_kernel = jnp.linspace(0, 2, 50)
    lu = tracer_utils()
    cosmo_h = cosmo_h_PL
...
z_many = jnp.linspace(0, 4, 5000)


def lsst_source_tomo_bins(
    zmin=0.1,
    zmax=3,
    ns0=27,
    nbins=3,
    ...
    **kwargs
    ...
    z = zp
    if zp is None:
        z = z_many[jnp.where(jnp.logical_and(z_many >= zmin, z_many <= zmax))]
    if pzp is None:
    ...
    z = z[x]
    pzp = pzp[x]
    ns0 = ns0 * jnp.sum(pzp) / N1
    print("ns0: ", ns0)

    if z_bins is None:
        z_bins = jnp.linspace(zmin, min(2, zmax), nbins + 1)
        z_bins = z_bins.at[-1].set(zmax)

    if z_bias is None:
        z_bias = jnp.zeros_like(z)
    else:
        try:
...
def DESI_lens_bins(
    ...
    **kwargs
    ...
    #     t=jnp.genfromtxt(home+fname,names=True,skip_header=3)
    t = jnp.genfromtxt(home + fname, names=("z_lo", "z_hi", "pz"))
...
def DES_lens_bins(
    fname="~/Cloud/Dropbox/DES/2pt_NG_mcal_final_7_11.fits",
    l=None,
    sigma_gamma=0,
    nside=256,
    mask_start_pix=0,
    ...
    try:
        ...
        t = jnp.genfromtxt(
            fname, names=("Z_MID", "BIN1", "BIN2", "BIN3", "BIN4", "BIN5")
        )
        dz = jnp.gradient(t["Z_MID"])
        zmax = max(t["Z_MID"]) + dz[-1] / 2.0

    nz_bins = 5
    nz = [0.0134, 0.0343, 0.0505, 0.0301, 0.0089]
    bz = [1.44, 1.70, 1.698, 1.997, 2.058]
    z_bins["SN"] = {}
    z_bins["SN"]["galaxy"] = jnp.zeros((len(l), nz_bins, nz_bins))
    z_bins["SN"]["shear"] = jnp.zeros((len(l), nz_bins, nz_bins))
    ...
    for i in range(nz_bins):
        ...
        z_bins[i]["Norm"] = jnp.sum(z_bins[i]["pzdz"])
        z_bins["SN"]["galaxy"][:, i, i] = galaxy_shot_noise_calc(
            zg1=z_bins[i], zg2=z_bins[i]
        )
        z_bins[i]["lm"] = 1.0e7
...
def DES_bins(
    ...
    try:
        ...
        t = jnp.genfromtxt(fname, names=("Z_MID", "BIN1", "BIN2", "BIN3", "BIN4"))
        dz = jnp.gradient(t["Z_MID"])
        zmax = max(t["Z_MID"]) + dz[-1] / 2.0
    ...
    for i in range(nz_bins):
        ...
        z_bins["SN"]["shear"][:, i, i] = shear_shape_noise_calc(
            zs1=z_bins[i], zs2=z_bins[i], sigma_gamma=sigma_gamma
        )
        z_bins[i]["lm"] = 1.0e7
        z_bins[i]["b1"] = 1
        z_bins[i]["AI"] = 0
...
def Kids_bins(
    kids_fname="/home/deep/data/KiDS-450/Nz_DIR/Nz_DIR_Mean/Nz_DIR_z{zl}t{zh}.asc",
    l=None,
    sigma_gamma=0,
):
    zl = [0.1, 0.3, 0.5, 0.7]
    ...
    for i in range(nz_bins):
        z_bins[i] = {}
        t = jnp.genfromtxt(
            kids_fname.format(zl=zl[i], zh=zh[i]), names=("z", "pz", "pz_err")
        )
        z_bins[i]["z"] = t["z"]
        z_bins[i]["dz"] = 0.05
        z_bins[i]["nz"] = nz[i]
...
def cmb_bins(zs=1100, l=None):
    ...
    cmb = StageIVCMB(beam=3.0, noise=1.0, lMin=30.0, lMaxT=3.0e3, lMaxP=5.0e3)
...
def combine_zbins(z_bins1={}, z_bins2={}):
    ...
    if z_bins1["n_bins"] == 0 or z_bins2["n_bins"] == 0:
        return z_bins3
    j = 0
    for i in range(z_bins1["n_bins"], z_bins1["n_bins"] + z_bins2["n_bins"]):
    ...
    z_bins3["SN"] = {}
    z_bins3["SN"]["galaxy"] = jnp.zeros((nl, z_bins3["n_bins"], z_bins3["n_bins"]))
    z_bins3["SN"]["shear"] = jnp.zeros((nl, z_bins3["n_bins"], z_bins3["n_bins"]))
    j = 0
    for i in range(z_bins1["n_bins"]):
        z_bins3["SN"]["galaxy"][:, j, j] += z_bins1["SN"]["galaxy"][:, i, i]
        z_bins3["SN"]["shear"][:, j, j] += z_bins1["SN"]["shear"][:, i, i]
        j += 1
    for i in range(z_bins2["n_bins"]):
