...
def shear_shape_noise_calc(zs1=None,zs2=None,sigma_gamma=0):
    ...
    SN=SN0*jnp.sum(zs1['W']*zs2['W']*zs1['nz']*nz_F) #FIXME: this is probably wrong.
...
def set_window(shear_zbins={},f_sky=0.3,nside=256,mask_start_pix=0,window_cl_fact=None,unit_win=False,scheduler_info=None,mask=None,
    ...
    kappa0=Skylens(galaxy_zbins=shear_zbins,bin_cl=False,l_bins=None,l=l0,use_window=False,
...
#             cl_map/=jnp.sqrt(cl_t[0]) #this is important for shear map normalization in correlation functions.
        ...
        if delta_W:
            ...
            shear_zbins[i]['window_N']=shear_zbins[i]['window_N'].at[~mask].set(hp.UNSEEN)
        else:
            print('not using delta_W window')
            shear_zbins[i]['window_N']=jnp.sqrt(1./shear_zbins[i]['window'])
            shear_zbins[i]['window_N']=shear_zbins[i]['window_N'].at[~mask].set(hp.UNSEEN)
...
def zbin_pz_norm(shear_zbins={},bin_indx=None,zs=None,p_zs=None,ns=0,bg1=1,AI=0,
    ...
    x=jnp.absolute(p_zs)<1.e-10 #to avoid some numerical errors
    ...
    shear_zbins[i]['dz']=jnp.gradient(shear_zbins[i]['z']) if len(shear_zbins[i]['z'])>1 else 1
...
#         crash
    ...
    for i in range(nz_bins):
        if use_shot_noise:
            shear_zbins['SN']['galaxy']=shear_zbins['SN']['galaxy'].at[:,i,i].set(galaxy_shot_noise_calc(zg1=shear_zbins[i],zg2=shear_zbins[i]))
            shear_zbins['SN']['shear']=shear_zbins['SN']['shear'].at[:,i,i].set(shear_shape_noise_calc(zs1=shear_zbins[i],zs2=shear_zbins[i],
                                                                 sigma_gamma=sigma_gamma))
            shear_zbins['SN']['kappa']=shear_zbins['SN']['kappa'].at[:,i,i].set(shear_shape_noise_calc(zs1=shear_zbins[i],zs2=shear_zbins[i],
...
def lens_wt_tomo_bins(zp=None,p_zp=None,nz_bins=None,ns=26,ztrue_func=None,zp_bias=None,
    ...
    for i in range(nz_bins):
        ...
        shear_zbins[i]['lens_kernel']=jnp.dot(shear_zbins[i]['pzdz'],sc)/shear_zbins[i]['Norm']
    shear_zbins['n_bins']=nz_bins #easy to remember the counts
...
def galaxy_tomo_bins(zp=None,p_zp=None,nz_bins=None,ns=10,ztrue_func=None,zp_bias=None,
    ...
    for i in range(nz_bins):
        ...
        galaxy_zbins[i]['dz']=jnp.gradient(galaxy_zbins[i]['z']) if len(galaxy_zbins[i]['z'])>1 else 1
...
def DES_lens_bins(fname='~/Cloud/Dropbox/DES/2pt_NG_mcal_final_7_11.fits',l=None,sigma_gamma=0,nside=256,mask_start_pix=0,window_cl_fact=0,unit_win=True,
    ...
    try:
        ...
        t=jnp.genfromtxt(fname,names=('Z_MID','BIN1','BIN2','BIN3','BIN4','BIN5'))
...
def Kids_bins(kids_fname='/home/deep/data/KiDS-450/Nz_DIR/Nz_DIR_Mean/Nz_DIR_z{zl}t{zh}.asc',l=None,sigma_gamma=0):
    ...
    for i in range(nz_bins):
        z_bins[i]={}
        t=jnp.genfromtxt(kids_fname.format(zl=zl[i],zh=zh[i]),names=('z','pz','pz_err'))
        z_bins[i]['z']=t['z']
        z_bins[i]['dz']=0.05
        z_bins[i]['nz']=nz[i]
        z_bins[i]['pz']=t['pz']
...
def cmb_bins(zs=1100,l=None):
    ...
    shear_zbins=zbin_pz_norm(shear_zbins=shear_zbins,bin_indx=0,zs=jnp.atleast_1d(zs),p_zs=jnp.atleast_1d(1),
                         ns=0,bg1=1)
