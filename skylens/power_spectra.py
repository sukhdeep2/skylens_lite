...
Bins_z_eagle = jnp.array(
    [3.53, 3.02, 2.48, 2.24, 2.01, 1.74, 1.49, 1.26, 1.0, 0.74, 0.5, 0.27, 0.0]
)
Bins_z_OWLS = jnp.array(
    [
        3.5,
        3.25,
        3.0,
...
class_accuracy_settings = {  # from Vanessa. To avoid class errors due to compiler issues.
    ...
    "k_max_tau0_over_l_max": 3.0,  # you can also try 5 here
    "k_step_sub": 0.015,
    "k_step_super": 0.0001,
    ...
    "tol_perturb_integration": 1.0e-4,
    "halofit_k_per_decade": 3000.0,  # you could try change this
}
...
class Power_Spectra(cosmology):
    def __init__(
        ...
        if not pk_params is None:
            self.kh = jnp.logspace(
                jnp.log10(pk_params["kmin"]),
                jnp.log10(pk_params["kmax"]),
                pk_params["nk"],
            )
    ...
    def get_pk(
        self, z, cosmo_params=None, pk_params=None, return_s8=False, pk_lock=None
    ):
        ...
        if pk_lock is not None:
            ...
            time.sleep(
                0.2 * ri
            )  # prevents threads from deadlocking while trying to acquire the lock
            ...
            try:
                pk_lock.acquire(
                    timeout="5s"
                )  # FIXME: this leads to deadlocks. Using randoms in sleep helps, but doesn't come with any gurantees.
        ...
        outp = pk_func(
            z, cosmo_params=cosmo_params, pk_params=pk_params, return_s8=return_s8
        )
        if pk_lock is not None:
            ntries = 0
            while pk_lock.locked():
    ...
    def eh_pk(self, z, cosmo_params=None, pk_params=None, return_s8=False):
        ...
        kh = (
            self.kh
        )  # jnp.logspace(jnp.log10(pk_params['kmin']),jnp.log10(pk_params['kmax']),pk_params['nk'])
        k = kh * cosmo_params["h"]
    ...
    def ccl_pk(self, z, cosmo_params=None, pk_params=None, return_s8=False):
        ...
        for i in jnp.arange(nz):
            #             try:
            ps[i] = pyccl_pkf(cosmo_ccl, k, 1.0 / (1 + z[i]))
        #             except Exception as err:
    ...
    def camb_pk(self, z, cosmo_params=None, pk_params=None, return_s8=False):
        ...
        if cosmo_params["w"] != -1 or cosmo_params["wa"] != 0:
            pars.set_dark_energy(
                w=cosmo_params["w"], wa=cosmo_params["wa"], dark_energy_model="ppf"
            )

        pars.InitPower.set_params(
            ns=cosmo_params["ns"], r=0, As=cosmo_params["Ase9"] * 1.0e-9
        )  #
        if return_s8:
        ...
        pars.set_matter_power(
            redshifts=z_t, kmax=pk_params["kmax"], silent=self.silence_camb
        )
        ...
        results = camb.get_results(
            pars
        )  # This is the time consuming part.. pk add little more (~5%).. others are negligible.... error when run in parallel threads??

        kh, z2, pk = results.get_matter_power_spectrum(
            minkh=pk_params["kmin"], maxkh=pk_params["kmax"], npoints=pk_params["nk"]
        )
        z2 = jnp.array(z2)
    ...
    def baryon_pk(self, z, cosmo_params=None, pk_params=None, return_s8=False):
        ...
        out = self.camb_pk_too_many_z(
            z, cosmo_params=cosmo_params, pk_params=pk_params, return_s8=return_s8
        )  # FIXME: pk_func should be use input.
        pk = out[0]
        kh = out[1]

        non_linear = (
            pk_params["non_linear"] if pk_params.get("non_linear") is not None else 1
        )
        ...
        f_logPkRatio = interpolate.interp2d(
            zbin_logPkR[scenario], Bins_log_k_Mpc, Arr2D_logPkratio, kind="linear"
        )
