...
class wigner_transform:
    def __init__(
        ...
        logger=None,
        ncpu=None,
        wig_d_taper_order_low=None,
        wig_d_taper_order_high=None,
        scheduler_info=None,
        l_cut_weights=None,
        **kwargs
    ):
    ...
    def set_binned_theta(self, theta_bins=[]):
        ...
        self.theta_bins_center = client.scatter(
    ...
    def wig_d_smoothing(self, s1_s2):
        ...
        zeros = jn_zeros(
            bessel_order, max(self.wig_d_taper_order_low, self.wig_d_taper_order_high)
        )
        ...
        taper_f = np.cos(
            ...
            / (l_max_high[:, None] - l_max_low[:, None])
            * np.pi
            / 2.0
        )
    ...
    def cl_grid(self, l_cl=[], cl=[], wig_l=None, taper=False, **kwargs):
        ...
        if np.all(wig_l == l_cl):
            return cl
        cl_int = interp1d(l_cl, cl, bounds_error=False, fill_value=0, kind="linear")
        if wig_l is None:
            wig_l = self.l
        cl2 = cl_int(wig_l)
        return cl2
    ...
    def projected_correlation(
        ...
        **kwargs
        ...
        if (
            wig_d is None
        ):  # when using default wigner matrices, interpolate to ensure grids match.
    ...
    def projected_covariance(
        ...
        **kwargs
        ...
        cov = (
            (wig_d1 * np.sqrt(wig_norm))
            @ np.diag(cl2 * grad_l)
            @ (wig_d2 * (np.sqrt(wig_norm))).T
        )
    ...
    def skewness(
        self, l_cl=[], cl1=[], cl2=[], cl3=[], s1_s2=[], taper=False, **kwargs
    ):
        """
    ...
    def l_cut_coupling(
        ...
        **kwargs
        ...
        for (m1, m2) in s1_s2:
            ...
            th, theta_coupling_mat[(m1, m2)] = self.projected_covariance(
                l_cl=l_cl, cl_cov=lw, taper=taper, s1_s2=s, **kwargs
            )
...
def projected_correlation(norm=1, cl=[], wig_d=None):
    ...
    w = np.dot(
        wig_d * norm, cl
    )  # for binned wig_d norm is already applied. Otherwise it is (2*l+1)/4pi * dl
    return w
