...
class Angular_power_spectra:
    def __init__(
        ...
        self.cl_f = (l + 0.5) ** 2 / (
            l * (l + 1.0)
        )  # cl correction from Kilbinger+ 2017
        self.tracer = tracer
    ...
    def set_z_PS(self, z=None, nz=None, log_z=None, z_max=None):
        ...
        if z is None:
            if log_z == 1:  # bins for z_lens.
                self.z = jnp.logspace(
                    jnp.log10(max(z_min, 1.0e-4)), jnp.log10(z_max), nz
                )
            elif log_z == 0:
                self.z = jnp.linspace(z_min, z_max, nz)
            else:
                z1 = jnp.logspace(
                    jnp.log10(max(z_min, 1.0e-4)), jnp.log10(z_max), int(nz / 2)
                )
                z2 = jnp.linspace(z_min, z_max, int(nz / 2))
                self.z = jnp.sort(
                    jnp.unique(jnp.around(jnp.append(z1, z2), decimals=3))
                )
        else:
            self.z = z
    ...
    def angular_power_z(
        self, z=None, pk_params=None, cosmo_h=None, cosmo_params=None, pk_lock=None
    ):
        """
        ...
        self.PS.get_pk(
            z=z, pk_params=pk_params, cosmo_params=cosmo_params, pk_lock=pk_lock
        )
        ...
        for i in range(nz):
            DC_i = chi[
                i
            ]  # cosmo_h.comoving_transverse_distance(z[i]).value#because camb k in h/mpc
            lz = kh * DC_i - 0.5
