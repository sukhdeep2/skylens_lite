...
class window_utils:
    def __init__(
        ...
        wigner_files=None,
        step=None,
        keep_wig3j_in_memory=None,  # xi_win_approx=False,
        client=None,
        scheduler_info=None,
        wigner_step=None,
        skylens_b_xi=None,
        bin_theta_window=None,
        njobs_submit_per_worker=10,
        zarr_parallel_read=25,
        skylens_class0=None,
        skylens_class_b=None,
        bin_window=None,
        do_pseudo_cl=None,
    ):
        ...
        self.__dict__.update(
            locals()
        )  # assign all input args to the class as properties
        ...
        self.MF = (
            2 * self.l_cl[:, None] + 1
        )  # this is multiplied with coupling matrices, before binning.
    ...
    def get_xi_win_WT(self):
        ...
        WT_kwargs = {
            "cl": {
                ...
                "grad_l": self.WT.grad_l,
            }
        }
        WT_kwargs = scatter_dict(
            WT_kwargs, broadcast=True, scheduler_info=self.scheduler_info, depth=2
        )
    ...
    def wig3j_step_read(self, m=0, lm=None, sem_lock=None):
        ...
        if sem_lock is None:
            out = wig_3j.oindex[
                jnp.int32(self.window_l),
                jnp.int32(self.l[lm : lm + step]),
                jnp.int32(self.l_cl),
            ]
        else:
            with sem_lock:
    ...
    def set_wig3j_step_multiplied(self, lm=None, sem_lock=None):
        ...
        wig_3j_2 = {}
        wig_3j_1 = {
            m1: self.wig3j_step_read(m=m1, lm=lm, sem_lock=sem_lock) for m1 in self.m_s
        }
        mi = 0
        for m1 in self.m_s:
            for m2 in self.m_s[mi:]:
                wig_3j_2[str(m1) + str(m2)] = {
                    0: wig_3j_1[m1] * wig_3j_1[m2].astype("float64")
                }  # numpy dot appears to run faster with 64bit ... ????
                #                 if m1!=0 or m2!=0:
                mf = self.set_window_pm_step(lm=lm)
                wig_3j_2[str(m1) + str(m2)][2] = (
                    wig_3j_2[str(m1) + str(m2)][0] * mf["mf_p"]
                )
                wig_3j_2[str(m1) + str(m2)][-2] = wig_3j_2[str(m1) + str(m2)][0] * (
                    1 - mf["mf_p"]
                )
            mi += 1
        #         del wig_3j_1
    ...
    def set_wig3j_step_spin(self, wig2, mf_pm, W_pm):
        ...
        if W_pm == 2:  # W_+
            mf = mf_pm[
                "mf_p"
            ]  # .astype('float64') #https://stackoverflow.com/questions/45479363/numpy-multiplying-large-arrays-with-dtype-int8-is-slow
        if W_pm == -2:  # W_+
            mf = 1 - mf_pm["mf_p"]
        return wig2 * mf
    ...
    def set_window_pm_step(self, lm=None):
        ...
        li2 = jnp.int32(self.l[lm : lm + self.step]).reshape(
            1, len(self.l[lm : lm + self.step]), 1
        )
        mf = (-1.0) ** (li1 + li2 + li3)
    ...
    def set_wig3j(self, set_wig_3j_2=False):
        ...
        m_s = jnp.concatenate(
            [jnp.abs(jnp.array(i)).flatten() for i in self.s1_s2s.values()]
        )
        ...
        if set_wig_3j_2:
            for lm in self.lms:  # set in get_coupling_lm_all_win
                wig_3j_2[lm] = delayed(self.set_wig3j_step_multiplied)(
                    lm=lm
                )  # ,sem_lock=self.sem_lock)
                wig_3j_2[lm] = client.compute(
                    wig_3j_2[lm], worker=self.workers_lm[lm], allow_other_workers=False
                )
    ...
    def coupling_matrix_large(
        self, win, wig_3j_2, mf_pm, bin_wt, W_pm, lm, cl_bin_utils=None
    ):
        """
        ...
        for k in win.keys():
            ...
            if self.bin_window:  # and bin_wt is not None:
                M[k] = self.binnings.bin_2d_coupling(
                    M=M[k],
                    bin_utils=cl_bin_utils,
                    partial_bin_side=2,
                    lm=lm,
                    lm_step=self.step,
                    wt0=bin_wt[k]["wt0"],
    ...
    def get_cl_coupling_lm(
        self, corr_indxs, win, lm, wig_3j_2_lm, mf_pm, cl_bin_utils=None
    ):
        """
        ...
        win_M = self.coupling_matrix_large(
            win[12],
            wig_3j_2=wig_3j_2,
            mf_pm=mf_pm,
            bin_wt=win["bin_wt"],
        ...
        if (
            win["corr"] == ("shear", "shear") and win["indxs"][0] == win["indxs"][1]
        ):  # B mode.
            win_M_B = self.coupling_matrix_large(
                win[12],
                wig_3j_2=wig_3j_2,
                mf_pm=mf_pm,
                W_pm=-2,
    ...
    def combine_coupling_cl(self, result):
        ...
        for ii_t in range(len(self.cl_keys)):  # list(result[0].keys()):
            ...
            for i_lm in range(len(self.lms)):
                ...
                result0["M"] = (
                    result0["M"].at[start_i:end_i, :].set(result[lm][ii]["M"])
                )
                ...
                if result_ii["M_noise"] is not None:
                    result0["M_noise"] = (
                        result0["M_noise"]
                        .at[start_i:end_i, :]
                        .set(result[lm][ii]["M_noise"])
                    )
                if corr == ("shear", "shear") and indxs[0] == indxs[1]:
                    result0["M_B_noise"] = (
                        result0["M_B_noise"]
                        .at[start_i:end_i, :]
                        .set(result[lm][ii]["M_B_noise"])
                    )
                    result0["M_B"] = (
                        result0["M_B"].at[start_i:end_i, :].set(result[lm][ii]["M_B"])
                    )
    ...
    def set_window_cl(
        ...
        if self.store_win and self.do_cl:
            ...
            if self.c_ell0 is not None:
                ...
                self.c_ell0 = scatter_dict(
                    self.c_ell0,
                    scheduler_info=self.scheduler_info,
                    broadcast=True,
                    depth=2,
                )
                self.c_ell_b = scatter_dict(
                    self.c_ell_b,
                    scheduler_info=self.scheduler_info,
                    broadcast=True,
                    depth=2,
            ...
            if self.xi0 is not None:
                ...
                self.xi0 = scatter_dict(
                    self.xi0,
                    scheduler_info=self.scheduler_info,
                    broadcast=True,
                    depth=2,
                )
                self.xi_b = scatter_dict(
                    self.xi_b,
                    scheduler_info=self.scheduler_info,
                    broadcast=True,
                    depth=2,
        ...
        if self.do_cl:
            ...
            def Win_cli():
                ...
                while i < len(self.cl_keys):
                    ...
                    indx = (ck[2], ck[3])
                    c_ell0 = (
                        {corr: {indx: self.c_ell0[corr][indx]}}
                        if self.c_ell0 is not None
                        else None
                    )
                    c_ell_b = (
                        {corr: {indx: self.c_ell_b[corr][indx]}}
                        if self.c_ell_b is not None
                        else None
                    )
                    yield delayed(get_window_power_cl)(
                        ck,
                        WU,
                        c_ell0=c_ell0,
                        c_ell_b=c_ell_b,
                        z_bin1=z_bins[ck[0]][ck[2]],
                        z_bin2=z_bins[ck[1]][ck[3]],
            ...
            if self.store_win:
                ...
                Win_cl = list(Win_cli())
            print(
                "set window_cl: cl done",
                time.time() - t1,
                get_size_pickle(self),
                get_size_pickle(WT_kwargs),
            )
    ...
    def set_store_window(
        ...
        if self.do_pseudo_cl:  # this is probably redundant due to if statement in init.
            ...
            for lm in self.lms:
                ...
                Win_lm[lm] = delayed(get_coupling_lm_all_win)(
                    WU, Win_cl, lm, wig_3j_2.get(lm), None, cl_bin_utils=cl_bin_utils
                )
                #### Donot delete
                if self.store_win:
                    client_func = client.compute
                    Win_lm[lm] = client_func(
                        Win_lm[lm],
                        priority=2,
                        workers=self.workers_lm[lm],
...
def get_cl_coupling_lm(WU, lm, wig_3j_2_lm, mf_pm, win, cl_bin_utils=None):
    ...
    return WU.get_cl_coupling_lm(
        None, win, lm, wig_3j_2_lm, mf_pm, cl_bin_utils=cl_bin_utils
    )
...
def get_coupling_lm_all_win(WU, Win_cl, lm, wig_3j_2_lm, mf_pm, cl_bin_utils=None):
    ...
    if self.do_cl:
        Win_lm["cl"] = [
            self.get_cl_coupling_lm(
                None, Wc, lm, wig_3j_2_lm, mf_pm, cl_bin_utils=cl_bin_utils
            )
            for Wc in Win_cl
        ]
...
def get_window_power_cl(
    ...
    win[12]["cl"] = jnp.asarray(
        hp.anafast(map1=z_bin1["window"], map2=z_bin2["window"], lmax=self.window_lmax)
    )[self.window_l]
    ...
    if corr[0] == corr[1] and indxs[0] == indxs[1]:
        ...
        if map1 is None:
            ...
            map1[mask] = hp.UNSEEN
        win[12]["N"] = jnp.asarray(hp.anafast(map1=map1, lmax=self.window_lmax))[
            self.window_l
        ]
    ...
    if self.bin_window and self.do_pseudo_cl:
        ...
        win["bin_wt"] = {}
        win["bin_wt"]["cl"] = {"wt_b": 1.0 / cl_b, "wt0": cl0}
    ...
    win["s1s2"] = s1s2
    if self.do_xi:
        th, win["xi"] = self.WT.projected_correlation(
            cl=win[12]["cl"], **WT_kwargs
        )  # this is ~f_sky
        win["xi_b"] = self.binning.bin_1d(
            xi=win["xi"], bin_utils=xi_bin_utils
        )  # xi_bin_utils[(0,0)]
    ...
    win[
        ...
        {}
    )  # self.coupling_matrix_large(win['cl'], s1s2,wig_3j_2=wig_3j_2,W_pm=W_pm)*(2*self.l[:,None]+1) #FIXME: check ordering
    win["M_noise"] = None
    return win
